...
func sayhelloName(w http.ResponseWriter, r *http.Request) {
	r.ParseForm()       //解析 url 傳遞的引數，對於 POST 則解析 HTTP 回應內容的主體（request body）
...
```
>**Tips**:
>Request 本身也提供了 FormValue()函式來取得使用者提交的引數。如 r.Form["username"]也可寫成 r.FormValue("username")。呼叫 r.FormValue 時會自動呼叫 r.ParseForm，所以不必提前呼叫。r.FormValue 只會回傳同名引數中的第一個，若引數不存在則回傳空字串。
